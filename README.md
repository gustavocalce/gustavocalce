<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:064e3b,100:0b2f24&height=180&section=header&text=Gustavo%20Calce%20da%20Silva&fontSize=32&fontColor=ffffff&fontAlignY=35&desc=Data%20Engineer%20%7C%20Backend%20Developer%20%7C%20Cloud%20Specialist&descAlignY=55" alt="header" />

  <p><strong>Python | Java | AWS | Airflow | Kubernetes</strong></p>

  <p>
    <a href="https://www.linkedin.com/in/gustavo-calce-12103a207">
      <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
    </a>
  </p>
</div>

---

| About (EN) | Sobre (PT-BR) |
| --- | --- |
| I build data pipelines and backend services with a focus on reliability, performance, and clean architecture. I enjoy working across data engineering and DevOps practices, from orchestration to deployment. | Desenvolvo pipelines de dados e servicos de backend com foco em confiabilidade, performance e arquitetura limpa. Gosto de trabalhar entre engenharia de dados e DevOps, da orquestracao ao deploy. |

---

## Skills

<div align="center">
  <img alt="Python" height="46" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" />
  <img alt="Java" height="46" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/java/java-original.svg" />
  <img alt="AWS" height="46" src="https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazonaws&logoColor=white" />
  <img alt="Apache Airflow" height="46" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/apacheairflow/apacheairflow-original.svg" />
  <img alt="Kubernetes" height="46" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/kubernetes/kubernetes-plain.svg" />
  <img alt="Docker" height="46" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/docker/docker-original.svg" />
  <img alt="Linux" height="46" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/linux/linux-original.svg" />
  <img alt="PostgreSQL" height="46" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postgresql/postgresql-original.svg" />
</div>

<div align="center">
  <img src="https://img.shields.io/badge/Data%20Engineering-064e3b?style=for-the-badge" alt="Data Engineering" />
  <img src="https://img.shields.io/badge/Backend-0b2f24?style=for-the-badge" alt="Backend" />
  <img src="https://img.shields.io/badge/DevOps-0a3a2b?style=for-the-badge" alt="DevOps" />
</div>

---

## Featured Projects

- docker-automation: automation utilities for container workflows.
- WhatsappAiAgent: experiments with AI-driven WhatsApp interactions.
- DataPipelineBrazilian-sDeputies: data pipeline project for Brazilian deputies data.

---

## GitHub Stats

<div align="center">
  <img src="github-metrics.svg" alt="GitHub metrics" />
</div>

---

## Contact

<div align="center">
  <a href="https://www.linkedin.com/in/gustavo-calce-12103a207">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
  </a>
</div>

<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:0b2f24,100:064e3b&height=120&section=footer" alt="footer" />
</div>
